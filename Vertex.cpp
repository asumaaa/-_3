...
void Ver3::Initialize(XMFLOAT3 size)
{
	...
	{
		if (i % 4 == 0) { v[i] = { {(((float)i / 4)		/ fineSize) * size.x, (((float)i / 4)		/ fineSize) * size.y, 0}, {}, {0.5f,0.0f} }; }
		if (i % 4 == 1) { v[i] = { {(((float)i / 4 + 1) / fineSize) * size.x, (((float)i / 4 + 1)	/ fineSize) * size.y, 0}, {}, {0.5f,0.0f} }; }
		if (i % 4 == 2) { v[i] = { {(((float)i / 4)		/ fineSize) * size.x, (((float)i / 4)		/ fineSize) * size.y, 0}, {}, {0.5f,0.0f} }; }
		if (i % 4 == 3) { v[i] = { {(((float)i / 4 + 1) / fineSize) * size.x, (((float)i / 4 + 1)	/ fineSize) * size.y, 0}, {}, {0.5f,0.0f} }; }
	}
	//インデックスデータ
	unsigned short in[fine * 4];
	for (int i = 0; i < fineSize * 4; i++)
	{
		in[i] = i;
	}
